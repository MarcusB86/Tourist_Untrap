...
app.get('/add-more', async (req, res) => {
  try {
    ...
    const totalCount = await Attraction.count();
    
    res.json({ 
      message: `Added ${addedCount} new attractions!`,
      attractionsAdded: addedCount,
      totalAttractions: totalCount
    });
    
  } catch (error) {
    console.error('Add more attractions error:', error);
    res.status(500).json({ 
      message: 'Failed to add attractions',
      error: error.message 
    });
  }
});
